...
describe("wallpaper URL helpers", () => {
  ...
  test("builds an OG wallpaper image path", () => {
    expect(buildWallpaperImagePath(1179, 2556, "view=days&theme=dark")).toBe(
      "/og/1179x2556?view=days&theme=dark",
    );
  });

  test("builds absolute URLs only after the browser origin is known", () => {
    expect(buildAbsoluteWallpaperUrl("", "/og/1179x2556?view=days")).toBe("");
    expect(buildAbsoluteWallpaperUrl("https://life.example", "/og/1179x2556?view=days")).toBe(
      "https://life.example/og/1179x2556?view=days",
    );
  });
});

...
- 2026-07-03 - Never use `overflow: hidden` in the OG route JSX: satori emits it as a clip mask and resvg becomes ~50x slower on phone-sized canvases (0.6s → 30-50s on Vercel, so browsers give up). A full-canvas wrapper doesn't need it — the image bounds already clip (verified pixel-identical output without it). There is no unit-test seam for this; verify with a timing curl against the route when touching wrapper styles.
- 2026-09-11 - `bun.lock` once listed only `@typescript/typescript-darwin-arm64` of TypeScript 7's platform binaries, so Linux CI/Vercel builds crashed with "Unable to resolve @typescript/typescript-linux-x64" once Next 16.3 started running the `typescript` compiler during `next build`. After touching `typescript` in the lockfile, check that all `@typescript/typescript-*` entries are present (`grep -c '"@typescript/typescript-' bun.lock` ≈ 20). Dependabot's bun updater may also bump `package.json` without regenerating `bun.lock`, which fails CI's `--frozen-lockfile` install.
